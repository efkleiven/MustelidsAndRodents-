...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    logit(gamA[b, j, t])  <- beta0_gamA[hab[b, j]]  + beta1_gamA[temp[ind[b]+j, t]] 
    logit(gamAB[b, j, t]) <- beta0_gamAB[hab[b, j]] + beta1_gamAB[temp[ind[b]+j, t]] 
    logit(gamB[b, j, t])  <- beta0_gamB[hab[b, j]]  + beta1_gamB[temp[ind[b]+j, t]] 
    logit(gamBA[b, j, t]) <- beta0_gamBA[hab[b, j]] + beta1_gamBA[temp[ind[b]+j, t]]
 
    logit(epsA[b, j, t])  <- beta0_epsA[hab[b, j]]  + beta1_epsA[temp[ind[b]+j, t]] 
    logit(epsAB[b, j, t]) <- beta0_epsAB[hab[b, j]] + beta1_epsAB[temp[ind[b]+j, t]]
    logit(epsB[b, j, t])  <- beta0_epsB[hab[b, j]]  + beta1_epsB[temp[ind[b]+j, t]]
    logit(epsBA[b, j, t]) <- beta0_epsBA[hab[b, j]] + beta1_epsBA[temp[ind[b]+j, t]]
...
# make indexin variable
ind <- c(0,12,12*2,12*3, 12*4, 12*5, 12*6, 12*7)
ind

# give data
data <-list(nseason = dim(yb)[3], nblock = dim(yb)[2], nsite = dim(yb)[1], nsurvey = dim(yb)[4], 
            nout=4, y = yb, hab=hab, temp=temp, ind=ind)
...
va_mustela_rodent_habtemp_ni5k <- jags(data, inits=inits, params, "mod_seas_det_4stpm.txt", n.chains = nc,
                                   n.thin = nt, n.iter = ni, n.burnin = nb, n.adapt=na, parallel = T)

# Save model
setwd("./model_output")
save(va_mustela_rodent_habtemp_ni5k, file="va_mustela_rodent_habtemp_ni5k.rda")
